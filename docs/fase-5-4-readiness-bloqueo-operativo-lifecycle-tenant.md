...
| `suspended` | Bloquear nuevas mutaciones sensibles del tenant; permitir cierres/finalizaciones seguras de operaciones ya iniciadas cuando evitan datos abiertos o riesgo operativo. | Conservar histórico y consultas necesarias. | Permitido para gestión, soporte y auditoría. | Reactivar permitido a `superadmin`/`platform_ops`; archivar solo `superadmin`. |
| `archived` | Bloquear nuevas mutaciones y acceso tenant operativo; evaluar solo cierres terminales indispensables de registros abiertos heredados antes del archivo. | Conservar histórico autorizado; preferir vistas/RPC read-only plataforma. | Permitido para plataforma autorizada. | Terminal: no permitir transiciones salientes según FASE 5.2. |
...
- Crear visitas y registrar ingresos/accesos nuevos. Las salidas o cierres terminales de visitas ya abiertas deben tratarse como finalización segura, no como operación nueva.
...
| `tenant_terminal_close` | Solo si cierra operación iniciada en onboarding y autorizada | Sí | Sí, si no crea actividad nueva y deja historial consistente | Solo caso heredado indispensable y autorizado |
...
| --- | --- | --- | --- |
| Visitas — `registro_visitas`, `visitantes`, `accesos` | Historial de visitas/accesos autorizado por RLS. | Crear visitas, registrar ingresos/accesos nuevos y reabrir operaciones. Deben permitirse salidas/cierres terminales de registros ya existentes y abiertos para mantener historial exacto (`registro_visitas.hora_salida`/`estado`, `accesos.fecha_salida`). | Primer lote recomendado por impacto operativo claro; requiere distinguir mutación nueva vs cierre seguro de operación en curso. |
...
| Reservas — `reservas`, `reservas_zonas`, `reservas_bloqueos`, `reservas_documentos`, `reservas_eventos` | Historial de reservas y soportes. | Crear reservas, bloquear zonas, adjuntar documentos operativos o cambiar estados que creen operación nueva. | Segundo lote; revisar constraint de solape y permitir cancelaciones/cierres seguros de operaciones en curso cuando reduzcan riesgo. |
...
| Trasteos — `trasteos` | Historial de trasteos. | Crear, aprobar o reprogramar trasteos operativos nuevos. | Bloquear nuevas solicitudes; definir cancelación/cierre seguro para trasteos ya aprobados o en curso. |
...
2. **FASE 5.4.2 — Primer lote de mutaciones de bajo acoplamiento**
   ...
   - En visitas/accesos, bloquear nuevas visitas e ingresos, pero permitir salida/cierre terminal de registros ya abiertos antes del bloqueo.
   - Agregar pruebas negativas para `suspended` y `archived` y pruebas positivas de cierres seguros.
...
- Tenant `suspended`: mutación nueva falla sin insertar/actualizar datos, salvo cierre/finalización segura explícitamente permitida.
- Tenant `archived`: mutación nueva falla sin insertar/actualizar datos; cualquier cierre terminal heredado requiere caso documentado y autorización explícita.
...
- Operación en curso antes de suspensión puede cerrarse de forma terminal cuando el cierre mejora consistencia histórica y no crea nueva actividad.
- Si se auditan intentos bloqueados en una fase futura, la auditoría no debe contener PII innecesaria.


### Caso documental obligatorio para visitas/accesos

Escenario a cubrir antes de implementar el primer lote:

1. Crear o identificar una visita en tenant `active` y registrar su ingreso antes de la suspensión.
2. Cambiar el tenant a `suspended` mediante la RPC lifecycle autorizada.
3. Intentar registrar un nuevo ingreso o crear una nueva visita: debe fallar sin efectos parciales.
4. Registrar la salida/cierre terminal de la visita ya abierta: debe permitirse y actualizar campos terminales como `registro_visitas.hora_salida`/`estado` y/o `accesos.fecha_salida`.
5. Verificar que el historial quede consistente, sin visitas abiertas artificialmente y sin crear nueva actividad operativa.

Este criterio debe revisarse conceptualmente en otros módulos con operaciones en curso: cancelaciones, cierres, finalizaciones seguras o reversos que reducen riesgo no siempre equivalen a nuevas mutaciones y deben definirse por módulo antes de bloquear.

## Criterios no-go

No avanzar de DEV a QA/producción si ocurre cualquiera de estos casos:

- El helper permite mutaciones en `suspended` o `archived` para operaciones tenant no exceptuadas, o bloquea cierres terminales que dejan registros abiertos/inconsistentes.
...
- Si se detecta bloqueo indebido de lecturas o de cierres terminales seguros, revertir el lote que conectó el módulo, no el modelo lifecycle completo.
...
- Tienen separación relativamente clara entre lectura histórica, creación/ingreso nuevo y salida/cierre terminal seguro.
...
- Validar `registro_visitas`, `visitantes` y `accesos` para bloquear creación/ingreso nuevo en tenants bloqueados y permitir salida/cierre terminal de registros abiertos preexistentes.
...
| 5.4.2 | Bloqueo backend visitas/paquetes | Migración/policies/RPCs acotadas + docs | Pruebas negativas `active/suspended/archived` y prueba positiva de salida/cierre de visita abierta antes de suspensión |
