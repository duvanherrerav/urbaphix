...
El cambio técnico central es `src/services/membershipResolver.js`, que resuelve un perfil compatible con la forma que el frontend ya consume (`usuarioApp.id`, `usuarioApp.rol_id`, `usuarioApp.conjunto_id` y, cuando aplica, `usuarioApp.residente_id`). Su activación queda controlada por el feature flag público `VITE_ENABLE_MEMBERSHIP_RESOLVER`.
...
La lectura de `tenant_memberships` queda apagable con `VITE_ENABLE_MEMBERSHIP_RESOLVER`.
...
## Nuevo flujo compatible con flag encendido

1. `src/App.jsx` obtiene el usuario autenticado igual que antes.
2. Si `VITE_ENABLE_MEMBERSHIP_RESOLVER` está apagado, `App.jsx` consulta `usuarios_app` directamente y termina el flujo legacy.
3. Si el flag está encendido, `src/services/membershipResolver.js` consulta en paralelo:
   - `tenant_memberships` por `user_id` y `status = 'active'`.
   - `usuarios_app` por `id`, como fuente legacy y fallback.
4. Si existe una membership activa y compatible, el resolver construye un perfil compatible con el frontend actual.
5. Si no existe membership activa compatible, si la consulta falla por RLS/red, o si los datos mínimos no son seguros, el resolver vuelve a `usuarios_app`.
6. La navegación principal permanece estable porque sigue usando `usuarioApp.rol_id`.
...
- **Usuario sin membership activa:** usa `usuarios_app` si existe y registra `no_active_membership`.
- **Error al consultar `tenant_memberships`:** no bloquea login; usa fallback legacy, registra `tenant_memberships_query_failed` y emite warning solo en desarrollo.
- **Usuario legacy sin registro en `tenant_memberships`:** mantiene comportamiento anterior con `usuarios_app`.
- **Múltiples memberships activas:** selecciona primero la membership cuyo `conjunto_id` coincida con `usuarios_app.conjunto_id`; si no hay coincidencia, usa la primera por `created_at` y registra `multiple_active_memberships_detected` solo en desarrollo.
- **Membership sin `conjunto_id`:** se considera incompatible, registra `membership_missing_conjunto_id` y cae a legacy.
- **Membership residente sin `residente_id`:** se considera incompleta para transición, registra `resident_membership_missing_residente_id` y cae a legacy.
- **Rol de membership sin mapeo UI:** se considera incompatible en esta fase, registra `membership_role_not_supported_by_current_navigation` y cae a legacy.
...
### Con `VITE_ENABLE_MEMBERSHIP_RESOLVER` apagado o ausente

- Login admin legacy exitoso consultando `usuarios_app`.
- Login vigilancia legacy exitoso consultando `usuarios_app`.
- Login residente legacy exitoso consultando `usuarios_app`.
- Dashboard admin carga.
- Control visitas vigilancia carga.
- Solicitar visita residente carga.
- Network no muestra consultas del bootstrap/login a `tenant_memberships`.

### Con `VITE_ENABLE_MEMBERSHIP_RESOLVER` encendido

- Login admin con membership `admin_conjunto` activa resuelve `admin`.
- Login vigilancia con membership `vigilante` activa resuelve `vigilancia`.
- Login residente con membership `residente` activa y `residente_id` resuelve `residente`.
- Dashboard admin carga.
- Control visitas vigilancia carga.
- Solicitar visita residente carga.
...
- Fallback legacy funciona si una membership activa no tiene `conjunto_id`, si un residente no tiene `residente_id`, o si el rol no está soportado por la navegación actual.
