...
## Resumen ejecutivo
Esta subfase reduce superficie pública en Supabase revocando `EXECUTE` al rol `anon` **y también a `PUBLIC`** sobre helpers de autorización/RLS (modernos y legacy). Esto evita acceso anónimo heredado por membresía implícita en `PUBLIC`. Se mantienen sin cambios los grants de `authenticated` y `service_role`, y no se modifican las RPC productivas de visitas/vigilancia.
...
## Qué cambia
- Nueva migración versionada que ejecuta, por cada helper objetivo:
