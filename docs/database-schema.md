...
El hotfix histórico `20260715120000_hotfix_qa_fn_platform_tenants_summary_qualified_conjunto_id.sql` se conserva con el mismo contenido que QA/PRD. Tras la auditoría semántica posterior a DEV, las definiciones versionadas y aplicadas en DEV constituyen el baseline canónico DEV/PRD para estos objetos. La migración forward-only `20260723120000_promote_canonical_qa_prd_function_rls.sql` prepara la promoción idempotente de las mismas definiciones a QA y luego PRD, pero no aplica cambios remotos por sí sola. Sus postchecks validan atributos, grants y predicados de policies normalizados, sin depender del formato de `pg_get_functiondef`. Las únicas diferencias admitidas por ambiente son datos, secretos y variables documentadas en `docs/issue-307-canonical-normalization.md`.
