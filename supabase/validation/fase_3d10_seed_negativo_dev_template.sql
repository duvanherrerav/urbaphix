...
-- FASE 3D.10A - Seed controlado DEV dataset negativo RLS con UUID reales Auth
-- Project ref DEV esperado: polstaxmencetxgctvsw
--
-- PRECONDICION MANUAL:
--   1) Crear y autoconfirmar en Supabase Dashboard DEV los auth.users de prueba:
--      dev-rls-negative-same@urbaphix.com
--      dev-rls-negative-cross@urbaphix.com
--   2) Ejecutar fase_3d10_diagnostico_pre_seed_negativo_dev.sql.
--   3) Copiar abajo los UUID reales generados por Supabase Auth DEV.
--   No asumir UUIDs fijos para auth.users creados desde Dashboard.
...
DECLARE
  ...
  -- Reemplazar manualmente con los UUID reales generados por Supabase Auth DEV.
  -- Obtenerlos con fase_3d10_diagnostico_pre_seed_negativo_dev.sql.
  ...
  v_tipo_documento text := 'DEV-RLS-NEGATIVE-DOC';
BEGIN
  ...
  IF v_usuario_same_tenant_id_text = '<UUID_REAL_AUTH_SAME_TENANT>' OR v_usuario_cross_tenant_id_text = '<UUID_REAL_AUTH_CROSS_TENANT>' THEN
    RAISE EXCEPTION 'ABORTADO: reemplazar placeholders UUID reales Auth antes de ejecutar el seed.';
  END IF;
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM auth.users au
    WHERE au.id = v_usuario_same_tenant_id
      AND lower(au.email) = lower(v_email_same_tenant)
      AND au.email_confirmed_at IS NOT NULL
  ) THEN
    RAISE EXCEPTION 'ABORTADO: Auth user same tenant % no existe, no tiene email % o no esta confirmado.', v_usuario_same_tenant_id, v_email_same_tenant;
  END IF;

  IF NOT EXISTS (
    SELECT 1
    FROM auth.users au
    WHERE au.id = v_usuario_cross_tenant_id
      AND lower(au.email) = lower(v_email_cross_tenant)
      AND au.email_confirmed_at IS NOT NULL
  ) THEN
    RAISE EXCEPTION 'ABORTADO: Auth user cross tenant % no existe, no tiene email % o no esta confirmado.', v_usuario_cross_tenant_id, v_email_cross_tenant;
  END IF;

  INSERT INTO public.tipos_documento (codigo, nombre, activo)
  VALUES (v_tipo_documento, 'DEV-RLS-NEGATIVE-DOC', true)
  ...
  VALUES
    (v_usuario_same_tenant_id, v_conjunto_principal_id, 'residente', 'DEV-RLS-NEGATIVE-RESIDENTE-SAME', NULL, true, v_email_same_tenant),
    (v_usuario_cross_tenant_id, v_conjunto_ajeno_id, 'residente', 'DEV-RLS-NEGATIVE-RESIDENTE-CROSS', NULL, true, v_email_cross_tenant)
  ON CONFLICT (id) DO UPDATE
    SET conjunto_id = EXCLUDED.conjunto_id,
  ...
  VALUES
    (v_residente_same_id, v_usuario_same_tenant_id, false, v_conjunto_principal_id, v_apto_same_id),
    (v_residente_cross_id, v_usuario_cross_tenant_id, false, v_conjunto_ajeno_id, v_apto_cross_id)
  ON CONFLICT (id) DO UPDATE
  ...
  INSERT INTO public.tenant_memberships (user_id, conjunto_id, role_name, residente_id, status, source_legacy)
  VALUES
    (v_usuario_same_tenant_id, v_conjunto_principal_id, 'residente', v_residente_same_id, 'active', 'fase_3d10_negative_rls'),
    (v_usuario_cross_tenant_id, v_conjunto_ajeno_id, 'residente', v_residente_cross_id, 'active', 'fase_3d10_negative_rls')
  ON CONFLICT DO NOTHING;

  UPDATE public.tenant_memberships
  SET role_name = 'residente',
      residente_id = CASE
        WHEN user_id = v_usuario_same_tenant_id THEN v_residente_same_id
        WHEN user_id = v_usuario_cross_tenant_id THEN v_residente_cross_id
        ELSE residente_id
      END,
      source_legacy = 'fase_3d10_negative_rls',
      updated_at = now()
  WHERE status = 'active'
    AND (
      (user_id = v_usuario_same_tenant_id AND conjunto_id = v_conjunto_principal_id)
      OR (user_id = v_usuario_cross_tenant_id AND conjunto_id = v_conjunto_ajeno_id)
    );
  ...
  VALUES
    ...
    (v_paquete_cross_id, v_conjunto_ajeno_id, v_residente_cross_id, 'DEV-RLS-NEGATIVE-PAQUETE-CROSS', v_usuario_cross_tenant_id, 'pendiente', now(), NULL, v_apto_cross_id)
  ON CONFLICT (id) DO UPDATE
    SET conjunto_id = EXCLUDED.conjunto_id,
  ...
  VALUES (v_incidente_cross_id, v_conjunto_ajeno_id, v_usuario_cross_tenant_id, 'DEV-RLS-NEGATIVE-INCIDENTE-CROSS', 'bajo')
  ON CONFLICT (id) DO UPDATE
    SET conjunto_id = EXCLUDED.conjunto_id,
