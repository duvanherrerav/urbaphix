...
-- Solo SELECT: verifica privilegios EXECUTE por rol y funciones fuera de alcance

SELECT
  routine_schema,
  routine_name,
  has_function_privilege('anon', format('%I.%I()', routine_schema, routine_name), 'EXECUTE') AS anon_execute,
  has_function_privilege('authenticated', format('%I.%I()', routine_schema, routine_name), 'EXECUTE') AS authenticated_execute,
  has_function_privilege('service_role', format('%I.%I()', routine_schema, routine_name), 'EXECUTE') AS service_role_execute
