...
-- 0) Validación focal: PUBLIC/anon/authenticated en funciones internas objetivo.
SELECT
...
WHERE n.nspname = 'public'
  AND p.proname IN ('rls_auto_enable', 'set_updated_at', 'handle_new_user')
...
-- 2) SECURITY DEFINER aún ejecutables por PUBLIC/anon/authenticated.
SELECT
  n.nspname AS schema_name,
  p.proname AS function_name,
  pg_get_function_identity_arguments(p.oid) AS identity_args,
...
-- 3) Funciones trigger/event-trigger expuestas a PUBLIC/anon/authenticated.
SELECT
  n.nspname AS schema_name,
  p.proname AS function_name,
  pg_get_function_identity_arguments(p.oid) AS identity_args,
